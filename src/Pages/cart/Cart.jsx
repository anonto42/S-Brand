...
function Cart() {
  ...
  return (
    <Layout >
      <div className="h-[auto] pb-[10%] bg-gray-100 pt-28 " style={{ backgroundColor: mode === 'dark' ? '#282c34' : '', color: mode === 'dark' ? 'white' : '', }}>
        <h1 className="mb-10 text-center text-2xl font-bold">Cart Items</h1>
        <div className="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0 ">
          <div className="rounded-lg md:w-2/3">

            {cartItems.map((item,index)=>{
              totalAmout += Number(item.price) ;
              return(
              ...
              <div className="sm:ml-4 sm:flex sm:w-full sm:justify-between">
                ...
                <div onClick={()=>dC(item)} className="mt-4 cursor-pointer flex justify-between sm:space-y-6 sm:mt-0 sm:block sm:space-x-6">
          ...
          <div className="mt-6 h-full rounded-lg border bg-white p-6 shadow-md md:mt-0 md:w-1/3" style={{ backgroundColor: mode === 'dark' ? 'rgb(32 33 34)' : '', color: mode === 'dark' ? 'white' : '', }}>
            <div className="mb-2 flex justify-between">
              <p className="text-gray-700" style={{ color: mode === 'dark' ? 'white' : '' }}>Total Items</p>
              <p className="text-gray-700" style={{ color: mode === 'dark' ? 'white' : '' }}>{cartItems.length}</p>
            </div>
            ...
            <div className="flex justify-between mb-3">
              ...
              <div className>
                <p className="mb-1 text-lg font-bold" style={{ color: mode === 'dark' ? 'white' : '' }}>{totalAmout}$</p>
              </div>
            </div>                  
              <Modal/>
          </div>
